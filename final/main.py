...
from agent_aggregator_dqn import AggregatorAgent
from agent_customer import CustomerAgent
from environment import Environment
from params import EPISODES, TIME_STEPS_TEST, TRAINING_START_DAY, NUM_RL_AGENTS, AGENT_IDS, TIME_STEPS_TRAIN, TRAINING_PERIOD, TRAINING_END_DAY ,DISSATISFACTION_COEFFICIENTS, REPLACE_TARGET_INTERVAL, TESTING_START_DAY, TESTING_END_DAY, BASELINE_START_DAY, \
    TESTING_PERIOD


env = Environment(AGENT_IDS, heterogeneous=True, baseline=False)
aggregator_agent = AggregatorAgent(env)
# Create the list to hold all customer agents
customer_agents = []

# Loop for creating agent instances
for agent_id,data_id in enumerate(AGENT_IDS):
    agent = CustomerAgent(agent_id, data_id, env, dummy=False, q_network=None, target_network=None)
...
def train(log, save):
    ...
    MAX_EPISODE_STEPS = 10   #MAX_EPISODE_STEPS = 1000 
    for episode in range(EPISODES):
        ...
        episode_steps = 0  # Initialize episode step count
        while episode_steps < MAX_EPISODE_STEPS:  # Check for maximum episode steps
        ...
        if episode % 1 == 0:
            ...
            print('Day:', datetime.datetime.strptime('{} {}'.format(env.day, 2022), '%j %Y'))
...
def test_single_day(path, day=TRAINING_START_DAY):
    ...
    time_labels = [datetime.datetime(year=2022, month=1, day=1) + datetime.timedelta(days=day - 1, minutes=i * 10) for i in range(TIME_STEPS_TEST)]
...
def main():
    ...
    day = 151  # Assign a valid day value here
    test_single_day(path=None, day=day)
...
def test_average(path):
    ...
    for day in range(TESTING_START_DAY, TESTING_END_DAY):
        print('Test on', datetime.datetime.strptime('{} {}'.format(day, 2022), '%j %Y'))
        env.reset(day=day, max_steps=TIME_STEPS_TEST)
    ...
    print('Path:', str(path))
    print('Test period', datetime.datetime.strptime('{} {}'.format(TESTING_START_DAY, 2022), '%j %Y'),
          datetime.datetime.strptime('{} {}'.format(TESTING_END_DAY, 2022), '%j %Y'))
...
def print_metrics(run_time, path):
    ...
    test_day = datetime.datetime.strptime('{} {}'.format(env.day, 2022), '%j %Y')
...
def plot_single_load_curve(plot_agent, time_labels, day, path):
    ...
    ax.set_ylabel('Power (W)', fontsize=32)
...
def plot_dissatisfaction(plot_agent, time_labels, path, plot_incentive):
    ...
    ax.stackplot(time_labels, b, c, d, e, a, labels=['cleaning_system', 'cooling_system', 'lightning_system', 'milk_pump', 'RO'])
...
def plot_schedule(plot_agent, time_labels, path):
    ...
    for i, (time_label, req, act) in enumerate(zip(time_labels, requests, actions)):
        ...
        for j, (dev_name, dev_color, dev_req, dev_act) in enumerate(zip(['cleaning_system', 'cooling_system', 'lightning_system', 'milk_pump', 'RO'], ['C4', 'C0', 'C1', 'C2', 'C3'], req, act)):
            height = 0.4
...
def init_plot(twinx=True):
    ...
    minute_locator = mdates.MinuteLocator(interval=10)
